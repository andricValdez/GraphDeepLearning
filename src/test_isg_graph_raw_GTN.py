...
from vanilla_gtn import EnhancedHybridISG_GTN 
...
def add_lap_pe(data, k=8, undirected=True, attr_name="lap_pe"):
    """Add Laplacian PE with safety check for small graphs"""
    from torch_geometric.transforms import AddLaplacianEigenvectorPE
    
    num_nodes = data.num_nodes if hasattr(data, 'num_nodes') else data.x.size(0)
    
    # Safety check: graph must have more nodes than k
    if num_nodes < 2 or num_nodes <= k:
        data.lap_pe = torch.zeros(num_nodes, k)
        return data
    
    # Try normal computation
    try:
...
class GNN(nn.Module):
    ...
    def build_conv_model(self, input_dim, hidden_dim, heads):
        if self.gnn_type == 'GCNConv':
            ...
            if self.edge_attr:
                return TransformerConv(input_dim, hidden_dim, heads=heads, edge_dim=self.edge_dim)
            else:
    ...
    def forward(self, x, edge_index, edge_attr=None, batch=None, token_dist=None, sentence_ids=None):
...
def main(dataset_name, cut_off_dataset, cuda_num=0, 
    ...
    if gnn_type == 'Vanilla-GTN-NoEdge':
        ...
        label = "Hybrid-ISG-GTN + Sentence-Aware" if use_sentence_aware else "Hybrid-ISG-GTN"
        print(f" {label}")
...
def compute_dep_frequency_features(pyg_data, graph):
    """
    VERSIÓN CORREGIDA: Usa pyg_data.dep_id directamente
    """
    from collections import Counter
    import torch
    
    # CRÍTICO: Usar dep_id de PyG (ya maneja duplicación de undirected)
    dep_types = pyg_data.dep_id.cpu().numpy()
    
    # Contar frecuencias
    dep_counts = Counter(dep_types)
    total_edges = len(dep_types)
    
    # Frecuencia para cada edge
    freq_per_edge = []
    for dep_id in dep_types:
        freq = dep_counts[dep_id] / total_edges
        freq_per_edge.append(freq)
    
    # Estadísticas
    freq_stats = {
        'dep_diversity': len(dep_counts) / 50.0,
        'max_freq': max(dep_counts.values()) / total_edges if dep_counts else 0,
        'entropy': compute_entropy(list(dep_counts.values()))
    }
    
    return torch.tensor(freq_per_edge, dtype=torch.float).unsqueeze(1), freq_stats



def compute_entropy(counts):
    """Calcula entropía de Shannon de las frecuencias"""
    import numpy as np
    counts = np.array(counts, dtype=float)
    if counts.sum() == 0:
        return 0.0
    counts = counts / counts.sum()
    counts = counts[counts > 0]  # evitar log(0)
    return -np.sum(counts * np.log2(counts))



def compute_dep_diversity_per_node(graph):
    """
    CORREGIDO: Mantiene el orden correcto de nodos
    """
    node_incoming_deps = {node: set() for node in graph.nodes()}
    node_outgoing_deps = {node: set() for node in graph.nodes()}
    
    for u, v, data in graph.edges(data=True):
        dep_type = data.get('gramm_relation', 'unk')
        node_outgoing_deps[u].add(dep_type)
        node_incoming_deps[v].add(dep_type)
    
    max_dep_types = 50.0
    
    # CRÍTICO: Usar graph.nodes() directamente para mantener orden
    diversity_features = []
    for node in graph.nodes():  # ← IMPORTANTE: orden consistente
        in_div = len(node_incoming_deps[node]) / max_dep_types
        out_div = len(node_outgoing_deps[node]) / max_dep_types
        diversity_features.append([in_div, out_div])
    
    return torch.tensor(diversity_features, dtype=torch.float)


def extract_sentence_ids_from_isg(graph):
    """Versión corregida compatible con grafos dirigidos y no dirigidos"""
    import torch
    import networkx as nx
    from collections import deque
    
    if not graph.has_node('ROOT_0'):
        return torch.zeros(graph.number_of_nodes(), dtype=torch.long)
    
    # CORREGIDO: Detectar tipo de grafo
    if isinstance(graph, nx.DiGraph):
        sentence_roots = list(graph.successors('ROOT_0'))
    else:
        sentence_roots = list(graph.neighbors('ROOT_0'))
    
    if len(sentence_roots) == 0:
        return torch.zeros(graph.number_of_nodes(), dtype=torch.long)
    
    node_to_sent_id = {}
    node_to_sent_id['ROOT_0'] = -1
    
    for sent_id, root in enumerate(sentence_roots):
        queue = deque([root])
        visited = set([root, 'ROOT_0'])
        node_to_sent_id[root] = sent_id
        
        while queue:
            node = queue.popleft()
            
            # CORREGIDO: Usar método apropiado según tipo de grafo
            if isinstance(graph, nx.DiGraph):
            ...
            for neighbor in neighbors:
                if neighbor not in visited:
                    ...
                    node_to_sent_id[neighbor] = sent_id
                    queue.append(neighbor)
    
    for node in graph.nodes():
        if node not in node_to_sent_id:
            node_to_sent_id[node] = 0
    
    sentence_ids = []
    for node in graph.nodes():
        sent_id = node_to_sent_id.get(node, 0)
        sentence_ids.append(max(0, sent_id))
    
    return torch.tensor(sentence_ids, dtype=torch.long)


def assign_sentence_ids_by_components(graph):
    """
    Fallback: Asignar sentence IDs basándose en componentes conectadas
    cuando no hay ROOT_0
    """
    import torch
    import networkx as nx
    ...
    # Asignar ID por componente
    node_to_sent_id = {}
    for comp_id, component in enumerate(components):
        for node in component:
            node_to_sent_id[node] = comp_id
...
#####********************************************************************* Refact main and extract_feat 


"""
MINIMAL INTEGRATION PATCH
=========================
Add this code to your existing script to enable feature caching.
Only modifies extract_feat() and main() - everything else stays the same.

INSTRUCTIONS:
1. Add this code at the end of your existing script
2. Replace your original extract_feat() function with extract_feat_cached()
3. Replace your original main() function with main_cached()
4. Keep all your existing helper functions and imports
"""
...
# ============================================================================
# FEATURE MANAGER (Lightweight version)
# ============================================================================

class SimpleFeatureCache:
    """Lightweight feature caching - just save/load operations"""
    
    def __init__(self, cache_dir, dataset_name, cutoff):
        self.cache_dir = Path(cache_dir) / dataset_name / cutoff
        self.cache_dir.mkdir(parents=True, exist_ok=True)
    
    def _get_path(self, feature_name, split):
        return self.cache_dir / f"{split}_{feature_name}.pt"
    
    def exists(self, feature_name, split):
        return self._get_path(feature_name, split).exists()
    
    def save(self, data, feature_name, split):
        path = self._get_path(feature_name, split)
        torch.save(data, path)
        logger.info(f"✅ Cached {feature_name} for {split} → {path}")
    
    def load(self, feature_name, split):
        path = self._get_path(feature_name, split)
        data = torch.load(path)
        logger.info(f"📂 Loaded {feature_name} for {split} from cache")
        return data
    
    def save_graphs(self, graphs, split):
        path = self.cache_dir / f"{split}_graphs.pkl"
        with open(path, 'wb') as f:
            pickle.dump(graphs, f)
        logger.info(f"✅ Cached graphs for {split}")
    
    def load_graphs(self, split):
        path = self.cache_dir / f"{split}_graphs.pkl"
        with open(path, 'rb') as f:
            graphs = pickle.load(f)
        logger.info(f"📂 Loaded graphs for {split} from cache")
        return graphs
    
    def graphs_exist(self, split):
        return (self.cache_dir / f"{split}_graphs.pkl").exists()


# ============================================================================
# REFACTORED EXTRACT_FEAT (with caching)
# ============================================================================

def assemble_features_from_cache(embeddings_list, pos_features_list, 
                                          domain_features_list, pe_list, config, i):
    """
    CORREGIDO: Valida dimensiones antes de concatenar
    """
    feats = [embeddings_list[i]]
    num_nodes = embeddings_list[i].size(0)
    
    # POS features
    if config.get('add_pos_feat', False) and pos_features_list is not None:
        pos_feat = pos_features_list[i]
        if pos_feat.size(0) == num_nodes:
            feats.append(pos_feat)
        else:
            print(f"⚠️  Skipping POS: size {pos_feat.size(0)} != {num_nodes}")
    
    # Domain features
    if config.get('add_domain_feat', False) and domain_features_list is not None:
        domain_feat = domain_features_list[i]
        if domain_feat.size(0) == num_nodes:
            feats.append(domain_feat)
        else:
            print(f"⚠️  Skipping domain: size {domain_feat.size(0)} != {num_nodes}")
    
    # Positional encodings
    if pe_list is not None and i < len(pe_list):
        pe_dict = pe_list[i]
        
        # Degree PE
        if config.get('use_degree_pe', False) and 'degree' in pe_dict:
            deg = pe_dict['degree']
            if deg.size(0) == num_nodes:
                feats.append(deg)
        
        # Laplacian PE
        if config.get('use_lap_pe', False) and 'lap_pe' in pe_dict:
            lap = pe_dict['lap_pe']
            if lap.size(0) == num_nodes:
                feats.append(lap)
        
        # Random Walk PE
        if config.get('use_rw_pe', False) and 'rw_pe' in pe_dict:
            rw = pe_dict['rw_pe']
            if rw.size(0) == num_nodes:
                feats.append(rw)
        
        # ISG Structural
        if config.get('use_isg_structural_pe', False) and 'isg_structural' in pe_dict:
            isg = pe_dict['isg_structural']
            for key in ['root_depth', 'betweenness', 'clustering', 'dep_diversity']:
                if key in isg and isg[key].size(0) == num_nodes:
                    feats.append(isg[key])
        
        # NUEVO: Dep diversity per node
        if config.get('use_dep_frequency', False) and 'dep_diversity_node' in pe_dict:
            dep_div = pe_dict['dep_diversity_node']
            if dep_div.size(0) == num_nodes:
                feats.append(dep_div)
            else:
                print(f"⚠️  Skipping dep_div: size {dep_div.size(0)} != {num_nodes}")
    
    return torch.cat(feats, dim=1)



def extract_feat_cached(graph_data, texts, labels, domains, device, tokenizer, model, projector,
                       pos_embedding, domain_embedding, domain2id, dep_embedding,
                       add_domain_feat=False, reduce_dim_emb=False, add_pos_feat=True,
                       project_after_concat=False, 
                       use_lap_pe=True, lap_k=8, undirected_for_lap=True,
                       use_degree_pe=True, use_rw_pe=True, rw_steps=(1,2,4,8),
                       use_isg_structural_pe=True, use_token_distance_attn=False,
                       use_global_pooling=False,
                       use_dep_frequency=True,
                       use_sentence_aware=True,
                       set="train",
                       cache=None,
                       force_rebuild=False):
    """
    VERSIÓN COMPLETA Y CORREGIDA con todos los fixes aplicados
    """
    
    if cache is None:
        logger.warning("⚠️  No cache provided - this will be slow!")
    ...
    # ==========================
    # STEP 1: Extract/Load LLM Embeddings
    # ==========================
    if not force_rebuild and cache.exists('llm_embeddings', set):
        llm_embeddings_list = cache.load('llm_embeddings', set)
    else:
        logger.info(f"🔨 Extracting LLM embeddings for {set}...")
        llm_embeddings_list = []
        for g_instance in tqdm(graph_data, desc=f"LLM embeddings ({set})"):
            graph = g_instance['graph']
            doc_id = g_instance['doc_id']
            node_feats = get_node_features_from_doc(graph, texts[doc_id], tokenizer, model, device)
            llm_embeddings_list.append(node_feats)
        cache.save(llm_embeddings_list, 'llm_embeddings', set)
    
    # ==========================
    # STEP 2: Project Embeddings
    # ==========================
    if reduce_dim_emb:
        if not force_rebuild and cache.exists('projected_embeddings', set):
            embeddings_list = cache.load('projected_embeddings', set)
        else:
            logger.info(f"🔨 Projecting embeddings for {set}...")
            embeddings_list = []
            for emb in tqdm(llm_embeddings_list, desc=f"Projection ({set})"):
                with torch.no_grad():
                    projected = projector(emb.to(device)).cpu()
                embeddings_list.append(projected)
            cache.save(embeddings_list, 'projected_embeddings', set)
    else:
        embeddings_list = llm_embeddings_list
    
    # ==========================
    # STEP 3: Extract/Load POS Features
    # ==========================
    if add_pos_feat:
        if not force_rebuild and cache.exists('pos_features', set):
            pos_features_list = cache.load('pos_features', set)
        else:
            logger.info(f"🔨 Extracting POS features for {set}...")
            pos_features_list = []
            for g_instance in tqdm(graph_data, desc=f"POS features ({set})"):
                graph = g_instance['graph']
                with torch.no_grad():
                    pos_ids = get_pos_encoding(graph)
                    pos_feats = pos_embedding(pos_ids.to(device)).cpu()
                pos_features_list.append(pos_feats)
            cache.save(pos_features_list, 'pos_features', set)
    else:
        pos_features_list = None
    
    # ==========================
    # STEP 4: Extract/Load Domain Features
    # ==========================
    if add_domain_feat:
        if not force_rebuild and cache.exists('domain_features', set):
            domain_features_list = cache.load('domain_features', set)
        else:
            logger.info(f"🔨 Extracting domain features for {set}...")
            domain_features_list = []
            for g_instance in tqdm(graph_data, desc=f"Domain features ({set})"):
                graph = g_instance['graph']
                doc_id = g_instance['doc_id']
                num_nodes = graph.number_of_nodes()
                ...
                with torch.no_grad():
                    domain_feat = domain_embedding(torch.tensor(source_id).to(device))
                    domain_feats = domain_feat.unsqueeze(0).repeat(num_nodes, 1).cpu()
                domain_features_list.append(domain_feats)
            cache.save(domain_features_list, 'domain_features', set)
    else:
        domain_features_list = None
    
    # ==========================
    # STEP 5: Extract/Load Edge Features + Dep Frequency
    # ==========================
    if not force_rebuild and cache.exists('edge_features', set):
        edge_features_list = cache.load('edge_features', set)
    else:
        logger.info(f"🔨 Extracting edge features for {set}...")
        edge_features_list = []
        for g_instance in tqdm(graph_data, desc=f"Edge features ({set})"):
            graph = g_instance['graph']
            graph = add_graph_attr(graph)
            pyg_data = from_networkx(graph)
            
            with torch.no_grad():
                dep_embed = dep_embedding(pyg_data.dep_id.to(device)).cpu()
            
            token_dist = extract_token_distances_aligned(graph, pyg_data)
            
            # CORREGIDO: Pasar pyg_data para mantener orden consistente
            if use_dep_frequency:
                # OPCIÓN 1: Usar la versión que recibe pyg_data (RECOMENDADO)
                dep_freq, freq_stats = compute_dep_frequency_features(pyg_data, graph)
                
                # OPCIÓN 2: Pasar edge_index a la versión original
                # dep_freq, freq_stats = compute_dep_frequency_features(graph, pyg_data.edge_index)
                
                # VERIFICACIÓN de dimensiones
                assert dep_freq.size(0) == dep_embed.size(0), \
                    f"Dep freq size {dep_freq.size(0)} != dep_embed size {dep_embed.size(0)}"
            else:
                dep_freq = None
                freq_stats = None
            ...
            edge_features_list.append({
                ...
                'dep_frequency': dep_freq,
                'freq_stats': freq_stats
            })
        cache.save(edge_features_list, 'edge_features', set)
        
    # ==========================
    # STEP 6: Extract/Load Sentence IDs
    # ==========================
    if use_sentence_aware:
        if not force_rebuild and cache.exists('sentence_ids', set):
            sentence_ids_list = cache.load('sentence_ids', set)
        else:
            logger.info(f"🔨 Extracting sentence IDs for {set}...")
            sentence_ids_list = []
            for g_instance in tqdm(graph_data, desc=f"Sentence IDs ({set})"):
                graph = g_instance['graph']
                sent_ids = extract_sentence_ids_from_isg(graph)
                sentence_ids_list.append(sent_ids)
            cache.save(sentence_ids_list, 'sentence_ids', set)
    else:
        sentence_ids_list = None
    
    # ==========================
    # STEP 7: Extract/Load Positional Encodings
    # ==========================
    pe_enabled = use_lap_pe or use_degree_pe or use_rw_pe or use_isg_structural_pe or use_dep_frequency
    
    if pe_enabled:
        if not force_rebuild and cache.exists('positional_encodings', set):
            pe_list = cache.load('positional_encodings', set)
        else:
            logger.info(f"🔨 Extracting positional encodings for {set}...")
            pe_list = []
            
            for idx, g_instance in enumerate(tqdm(graph_data, desc=f"PEs ({set})")):
                try:
                    graph = g_instance['graph']
                    num_nodes = graph.number_of_nodes()
                    
                    if num_nodes == 0:
                        logger.warning(f"⚠️  Graph {idx} is empty")
                        pe_list.append({})
                        continue
                    ...
                    # Degree PE
                    if use_degree_pe:
                        deg = degree(pyg_data.edge_index[0], num_nodes=num_nodes).unsqueeze(1)
                        pe_dict['degree'] = deg.cpu()
                    
                    # Laplacian PE
                    if use_lap_pe and num_nodes >= 2:
                        pyg_data = add_lap_pe(pyg_data, k=lap_k, undirected=undirected_for_lap, attr_name="lap_pe")
                        lap = torch.nan_to_num(pyg_data.lap_pe, nan=0.0, posinf=0.0, neginf=0.0).cpu()
                        pe_dict['lap_pe'] = lap
                    ...
                    if use_isg_structural_pe:
                        isg_pe = compute_isg_positional_encodings(graph, num_nodes)
                        dep_diversity_scalar = compute_dep_diversity_features(graph)
                        
                        pe_dict['isg_structural'] = {
                            'root_depth': isg_pe['root_depth'],
                            'betweenness': isg_pe['betweenness'],
                            'clustering': isg_pe['clustering'],
                            'dep_diversity': dep_diversity_scalar
                        }
                    
                    # Node-level dependency diversity
                    if use_dep_frequency:
                        dep_div_per_node = compute_dep_diversity_per_node(graph)
                        pe_dict['dep_diversity_node'] = dep_div_per_node
                    
                    pe_list.append(pe_dict)
                    
                except Exception as e:
                    logger.error(f"Error extracting PEs for graph {idx}: {e}")
                    import traceback
                    logger.error(traceback.format_exc())
                    pe_list.append({})
                    
            cache.save(pe_list, 'positional_encodings', set)
    else:
        pe_list = None
    
    # ==========================
    # STEP 8: Assemble PyG Data Objects
    # ==========================
    logger.info(f"📦 Assembling PyG Data objects for {set}...")
    data_list = []
    
    assembly_config = {
        'add_pos_feat': add_pos_feat,
        'add_domain_feat': add_domain_feat,
        'use_degree_pe': use_degree_pe,
        'use_lap_pe': use_lap_pe,
        'use_rw_pe': use_rw_pe,
        'use_isg_structural_pe': use_isg_structural_pe,
        'use_dep_frequency': use_dep_frequency
    }

    for i, g_instance in enumerate(tqdm(graph_data, desc=f"Assembly ({set})")):
        try:
            graph = g_instance['graph']
            doc_id = g_instance['doc_id']
            
            # CRÍTICO: Verificar que todos los índices coinciden
            if i >= len(embeddings_list):
                logger.error(f"Index {i} out of range for embeddings_list (len={len(embeddings_list)})")
                continue
            
            if set == "test":
                source_id = domain2id['unknown']
            else:
                source_id = domain2id.get(domains[doc_id], domain2id['unknown'])
            
            # Ensamblar features
            node_feats = assemble_features_from_cache(
                embeddings_list, pos_features_list, domain_features_list,
                pe_list, assembly_config, i
            )
            ...
            # Add edge features
            if i < len(edge_features_list):
                edge_feat = edge_features_list[i]
                pyg_data.edge_index = edge_feat['edge_index']
                
                if use_dep_frequency and edge_feat['dep_frequency'] is not None:
                    pyg_data.edge_attr = torch.cat([
                        edge_feat['dep_embed'],
                        edge_feat['dep_frequency']
                    ], dim=1)
                else:
                    pyg_data.edge_attr = edge_feat['dep_embed']
                
                pyg_data.token_distance = edge_feat['token_distance']
            
            # Add sentence IDs
            if use_sentence_aware and sentence_ids_list is not None and i < len(sentence_ids_list):
                pyg_data.sentence_ids = sentence_ids_list[i]
            
            data_list.append(pyg_data)
            
        except Exception as e:
            logger.error(f"Error assembling PyG data for doc_id={doc_id}: {e}")
            import traceback
            logger.error(traceback.format_exc())
            continue
    
    logger.info(f"✅ Assembled {len(data_list)} graphs for {set}")
    return data_list


def calculate_input_gnn_dim(lang_model, config):
    """Calculate correct input dimension based on config"""
    input_gnn_dim = config.get('reduced_dim', 256) if config.get('reduce_dim_emb') else lang_model.config.hidden_size
    
    if config.get('add_pos_feat'): input_gnn_dim += 32
    if config.get('add_domain_feat'): input_gnn_dim += 32
    if config.get('use_degree_pe'): input_gnn_dim += 1
    if config.get('use_lap_pe'): input_gnn_dim += config.get('lap_k', 8)
    if config.get('use_rw_pe'): input_gnn_dim += len(config.get('rw_steps', (1,2,4,8)))
    if config.get('use_isg_structural_pe'): input_gnn_dim += 4

    # NUEVO: Dependency diversity per node (2 features: in/out)
    if config.get('use_dep_frequency', False): input_gnn_dim += 2
    
    return input_gnn_dim


def calculate_edge_attr_dim_enhanced(base_dep_emb_dim, use_dep_frequency):
    """
    Calculate edge attribute dimension
    
    Args:
        base_dep_emb_dim: Original dependency embedding dimension (e.g., 32)
        use_dep_frequency: Whether to add frequency feature
    
    Returns:
        Total edge attribute dimension
    """
    edge_attr_dim = base_dep_emb_dim
    
    # NUEVO: +1 for dependency frequency
    if use_dep_frequency:
        edge_attr_dim += 1
    
    return edge_attr_dim




# ============================================================================
# REFACTORED MAIN FUNCTION (with caching)
# ============================================================================

def main_cached(dataset_name, cut_off_dataset, cuda_num=0, 
                graph_type='directed', edge_attr=False, 
                build_graph=True, max_features=None,
                reduce_dim_emb=True, reduced_dim=256,
                add_pos_feat=True, lr=0.0001, min_df=2,
                ...
                dropout=0.5, lang_model_name='microsoft/deberta-v3-base',
                project_after_concat=False, stop_words=False, special_chars=False,
                leave_out_sources=None, output_dir='',
                norm_type='batchnorm', post_mp_layers=2, pooling_type='mean', 
                balance_dataset=True,
                use_lap_pe=False, lap_k=8, undirected_for_lap=True,
                use_degree_pe=False, use_rw_pe=False, rw_steps=(1,2,4,8),
                use_isg_structural_pe=True, use_token_distance_attn=True,
                use_global_pooling=False,
                # NEW PARAMETERS
                use_cache=True,  # Enable caching
                force_rebuild=False,  # Force rebuild all features
                use_dep_frequency=True,  # NUEVO
                use_sentence_aware=True,  # NUEVO
                **kwargs):
    """
    Enhanced main function with feature caching
    
    NEW Args:
        use_cache (bool): If True, use feature caching (default: True)
        force_rebuild (bool): If True, rebuild all cached features (default: False)
    """
    ...
    if use_cache:
        cache = SimpleFeatureCache(
            ...
            cutoff=cut_off_dataset
        )
        logger.info(f"💾 Cache enabled: {cache.cache_dir}")
    else:
        cache = None
        logger.info("⚠️  Cache disabled - using original behavior")
    ...
    edge_attr_dim = calculate_edge_attr_dim_enhanced(dep_emb_dim, use_dep_frequency)
    ...
    # ========== GRAPH BUILDING (with caching) ==========
    
    if build_graph or (cache and not cache.graphs_exist('train')):
        logger.info("🔨 Building graphs...")
        
        # YOUR ORIGINAL GRAPH BUILDING CODE
        train_text_set, val_text_set, test_text_set = test_utils.read_dataset(dataset_name)
        ...
        cut_off_train, cut_off_val, cut_off_test = map(int, cut_off_dataset.split('_'))
        train_set = train_text_set[:int(len(train_text_set) * (cut_off_train / 100))]
        val_set = val_text_set[:int(len(val_text_set) * (cut_off_val / 100))]
        test_set = test_text_set[:int(len(test_text_set) * (cut_off_test / 100))]

        if balance_dataset:
            train_set = balance_df(train_set, "source")
            val_set = balance_df(val_set, "source")
        ...
        train_domains = list(train_set['source'])
        val_domains = list(val_set['source'])
        test_domains = list(test_set['source'])
        ...
        all_nlp_docs = train_nlp_docs + val_nlp_docs + test_nlp_docs
        vocab, word_to_index = create_vocab_v2(
            all_nlp_docs, stop_words=stop_words, special_chars=special_chars,
            min_df=min_df, max_features=max_features
        )

        isg = ISG(graph_type=graph_type, vocab=vocab, output_format='networkx',
                 apply_prep=True, parallel_exec=False, language='en', use_synonyms=False)

        graph_train, _, _ = isg.transform(train_nlp_docs)
        graph_val, _, _ = isg.transform(val_nlp_docs)
        graph_test, _, _ = isg.transform(test_nlp_docs)
        
        # Cache graphs
        if cache:
            cache.save_graphs(graph_train, 'train')
            cache.save_graphs(graph_val, 'val')
            cache.save_graphs(graph_test, 'test')
            
            # Cache metadata
            cache.save({'texts': train_texts, 'labels': train_labels, 'domains': train_domains}, 'metadata', 'train')
            cache.save({'texts': val_texts, 'labels': val_labels, 'domains': val_domains}, 'metadata', 'val')
            cache.save({'texts': test_texts, 'labels': test_labels, 'domains': test_domains}, 'metadata', 'test')
    
    else:
        logger.info("📂 Loading graphs from cache...")
        graph_train = cache.load_graphs('train')
        graph_val = cache.load_graphs('val')
        graph_test = cache.load_graphs('test')
        
        # Load metadata
        train_meta = cache.load('metadata', 'train')
        val_meta = cache.load('metadata', 'val')
        test_meta = cache.load('metadata', 'test')
        
        train_texts, train_labels, train_domains = train_meta['texts'], train_meta['labels'], train_meta['domains']
        val_texts, val_labels, val_domains = val_meta['texts'], val_meta['labels'], val_meta['domains']
        test_texts, test_labels, test_domains = test_meta['texts'], test_meta['labels'], test_meta['domains']
        
        # Rebuild domain2id
        train_set = pd.DataFrame({'source': train_domains, 'label': train_labels})
        val_set = pd.DataFrame({'source': val_domains, 'label': val_labels})
        test_set = pd.DataFrame({'source': test_domains, 'label': test_labels})
    ...
    llm_projector = build_projector(
        input_dim=input_proj_dim, hidden_dim=512, output_dim=reduced_dim,
        dropout=0.1, num_layers=1, use_norm=False, activation='gelu'
    ).to(device)

    pos_embedding = nn.Embedding(NUM_POS_TAGS, pos_emb_dim).to(device)
    domain_embedding = nn.Embedding(len(domain2id), domain_emb_dim).to(device)
    dep_embedding = nn.Embedding(NUM_DEP_TAGS, dep_emb_dim).to(device)

    # Use cached extract_feat
    data_train_list = extract_feat_cached(
        graph_train, train_texts, train_labels, train_domains, device, tokenizer,
        lang_model, llm_projector, pos_embedding, domain_embedding, domain2id, dep_embedding,
        add_domain_feat, reduce_dim_emb, add_pos_feat, project_after_concat,
        use_lap_pe, lap_k, undirected_for_lap, use_degree_pe, use_rw_pe, rw_steps,
        use_isg_structural_pe, use_token_distance_attn, use_global_pooling,
        set="train", cache=cache, force_rebuild=force_rebuild
    )
    
    data_val_list = extract_feat_cached(
        graph_val, val_texts, val_labels, val_domains, device, tokenizer,
        lang_model, llm_projector, pos_embedding, domain_embedding, domain2id, dep_embedding,
        add_domain_feat, reduce_dim_emb, add_pos_feat, project_after_concat,
        use_lap_pe, lap_k, undirected_for_lap, use_degree_pe, use_rw_pe, rw_steps,
        use_isg_structural_pe, use_token_distance_attn, use_global_pooling,
        set="val", cache=cache, force_rebuild=force_rebuild
    )
    
    data_test_list = extract_feat_cached(
        graph_test, test_texts, test_labels, test_domains, device, tokenizer,
        lang_model, llm_projector, pos_embedding, domain_embedding, domain2id, dep_embedding,
        add_domain_feat, reduce_dim_emb, add_pos_feat, project_after_concat,
        use_lap_pe, lap_k, undirected_for_lap, use_degree_pe, use_rw_pe, rw_steps,
        use_isg_structural_pe, use_token_distance_attn, use_global_pooling,
        set="test", cache=cache, force_rebuild=force_rebuild
    )

    # ========== REST OF YOUR ORIGINAL CODE (training, etc.) ==========
    
    train_loader = DataLoader(data_train_list, batch_size=batch_size, shuffle=True, num_workers=0)
    val_loader = DataLoader(data_val_list, batch_size=batch_size, shuffle=False, num_workers=0)
    test_loader = DataLoader(data_test_list, batch_size=batch_size, shuffle=False, num_workers=0)

    input_gnn_dim = calculate_input_gnn_dim(
        lang_model,
        config={
            'reduce_dim_emb': reduce_dim_emb,
    ...
    # Build model (YOUR ORIGINAL MODEL BUILDING CODE)
    if gnn_type == 'Vanilla-GTN-NoEdge':
        # Baseline: No edge features
        model = VanillaGTNWithEdgeConcat(
            input_dim=input_gnn_dim,
    ...
    elif gnn_type in ['TransformerConv']:
        model = GNN(
                input_dim = input_gnn_dim, 
                hidden_dim = hidden_gnn_dim, 
                dense_hidden_dim = dense_hidden_gnn_dim, 
    ...
    elif gnn_type == 'Enhanced-Hybrid-ISG-GTN':  # NUEVO
        model = EnhancedHybridISG_GTN(
            input_dim=input_gnn_dim,
            hidden_dim=hidden_gnn_dim,
            dense_hidden_dim=dense_hidden_gnn_dim,
            output_dim=2,
            ...
            use_token_distance=use_token_distance_attn,
            use_edge_bias=True,
            use_sentence_aware=use_sentence_aware,  # NUEVO
            heads=heads_gnn,
            norm_type=norm_type,
            pooling_type=pooling_type,
            post_mp_layers=post_mp_layers,
            edge_attr_dim=edge_attr_dim,  # DIMENSIÓN ACTUALIZADA
            ffn_mult=4,
            attn_dropout=0.1,
            use_global_pooling=use_global_pooling,
        ).to(device)
        print("🔥🆕 Enhanced Hybrid-ISG-GTN (+ Sentence-Aware + Dep Frequency)")
    ...
    for epoch in range(num_epochs):
        ...
        for data in train_loader:
            ...
            # MODIFICADO: Pasar sentence_ids si está disponible
            token_dist = data.token_distance if hasattr(data, 'token_distance') else None
            sentence_ids = data.sentence_ids if hasattr(data, 'sentence_ids') else None  # NUEVO
            
            out = model(data.x, data.edge_index, data.edge_attr, data.batch, 
                       token_dist, sentence_ids)  # NUEVO parámetro
...
1. BACKUP your original main() and extract_feat() functions

2. ADD this code to the END of your script

3. COPY your original extract_feat() body into extract_feat_original()

4. In your config or __main__, use main_cached() instead of main():

   # Old way (no caching):
   main(**config)
   
   # New way (with caching):
   main_cached(**config, use_cache=True)
   
   # Force rebuild:
   main_cached(**config, use_cache=True, force_rebuild=True)
   
   # Disable caching (use original behavior):
   main_cached(**config, use_cache=False)
...
if __name__ == "__main__":
    ...
    # Change hyperparameters only (uses cache):
    config['hidden_gnn_dim'] = 400
    config['lr'] = 0.00001
    main_cached(**config)  # Fast!
    
    # Force rebuild all features:
    main_cached(**config, force_rebuild=True)
"""
...
if __name__ == "__main__":
    ...
    if args.config_path:
        ...
        config = {
            ...
            'heads_gnn': 2,
            'dropout': 0.5,
            'lr': 0.000002, # 0001, 00001, 000001
            'gnn_type': 'Enhanced-Hybrid-ISG-GTN', # Vanilla-GTN-NoEdge', Vanilla-GTN-EdgeConcat, ISG-VanillaGTN, Hybrid-ISG-GTN, Enhanced-Hybrid-ISG-GTN, TransformerConv
            'norm_type': 'batchnorm',        # 'batchnorm', 'layernorm', or None
            ...
            'use_dep_frequency': True,      # NUEVO: Usar frecuencia de dependencias
            'use_sentence_aware': True,     # NUEVO: Usar atención sentence-aware
    ...
    # Fallback: standard run
        ...
        with mlflow.start_run(tags=run_tags):
            ...
            for k, v in config.items():
                mlflow.log_param(k, v)
            #main(**config)
            main_cached(**config, use_cache=True, force_rebuild=False)
