...
class GNN(nn.Module):
    ...
    def forward(self, x, edge_index, edge_attr=None, batch=None, token_dist=None):
...
def assemble_features_from_cache(embeddings_list, pos_features_list, domain_features_list,
                                 pe_list, config, i):
    """
    Assemble features based on config, selecting only requested PEs
    
    Args:
        embeddings_list: LLM or projected embeddings
        pos_features_list: POS features (or None)
        domain_features_list: Domain features (or None)
        pe_list: All positional encodings (dict with all PE types)
        config: Current experiment config
        i: Index of current graph
    
    Returns:
        node_feats: Concatenated features matching config
    """
    feats = [embeddings_list[i]]
    
    # Add POS features if requested
    if config.get('add_pos_feat', False) and pos_features_list is not None:
        feats.append(pos_features_list[i])
    
    # Add domain features if requested
    if config.get('add_domain_feat', False) and domain_features_list is not None:
        feats.append(domain_features_list[i])
    
    # Add ONLY requested PEs from cache
    if pe_list is not None and i < len(pe_list):
        pe_dict = pe_list[i]
        
        # Only add degree if requested
        if config.get('use_degree_pe', False) and 'degree' in pe_dict:
            feats.append(pe_dict['degree'])
        
        # Only add Laplacian if requested
        if config.get('use_lap_pe', False) and 'lap_pe' in pe_dict:
            feats.append(pe_dict['lap_pe'])
        
        # Only add RW if requested
        if config.get('use_rw_pe', False) and 'rw_pe' in pe_dict:
            feats.append(pe_dict['rw_pe'])
        
        # Only add ISG structural if requested
...
def extract_feat_cached(graph_data, texts, labels, domains, device, tokenizer, model, projector,
                       ...
                       set="train",
                       cache=None,  # NEW: SimpleFeatureCache instance
                       force_rebuild=False):  # NEW: Force rebuild flag
    """
    Cached version of extract_feat - saves/loads features independently
    
    Args:
        cache: SimpleFeatureCache instance (if None, behaves like original)
        force_rebuild: If True, ignore cache and rebuild everything
    """
    
    # If no cache, use original behavior
    if cache is None:
        return extract_feat_original(
            graph_data, texts, labels, domains, device, tokenizer, model, projector,
            pos_embedding, domain_embedding, domain2id, dep_embedding,
            add_domain_feat, reduce_dim_emb, add_pos_feat, project_after_concat,
            use_lap_pe, lap_k, undirected_for_lap, use_degree_pe, use_rw_pe, 
            rw_steps, use_isg_structural_pe, use_token_distance_attn, 
            use_global_pooling, set
        )
    ...
    # ==========================
    # STEP 2: Project Embeddings (if needed)
    # ==========================
    if reduce_dim_emb:
    ...
    # ==========================
    # STEP 5: Extract/Load Edge Features
    # ==========================
    ...
    # ==========================
    # STEP 6: Extract/Load Positional Encodings
    # ==========================
    pe_enabled = use_lap_pe or use_degree_pe or use_rw_pe or use_isg_structural_pe
    ...
    if pe_enabled:
        if not force_rebuild and cache.exists('positional_encodings', set):
            ...
            pe_list = []
            for g_instance in tqdm(graph_data, desc=f"PEs ({set})"):
                graph = g_instance['graph']
                num_nodes = graph.number_of_nodes()
                pyg_data = from_networkx(add_graph_attr(graph))
                ...
                # Laplacian PE
                if use_lap_pe and num_nodes >= 2:
                    pyg_data = add_lap_pe(pyg_data, k=lap_k, undirected=undirected_for_lap)
                ...
                if use_rw_pe and num_nodes >= 2:
                    rw = random_walk_landing_probs(
                        pyg_data.edge_index.to(device), num_nodes=num_nodes,
                        steps=rw_steps, device=device
                    ).cpu()
                    pe_dict['rw_pe'] = rw
                ...
                if use_isg_structural_pe:
                    isg_pe = compute_isg_positional_encodings(graph, num_nodes)
                    dep_diversity = compute_dep_diversity_features(graph)
    ...
    # ==========================
    # STEP 7: Assemble PyG Data Objects
    # ==========================
    logger.info(f"📦 Assembling PyG Data objects for {set}...")
    data_list = []
    
    # Build config for assembly
    assembly_config = {
...
# ============================================================================
# BACKUP: Rename your original extract_feat to extract_feat_original
# ============================================================================

def extract_feat_original(graph_data, texts, labels, domains, device, tokenizer, model, projector,
                          pos_embedding, domain_embedding, domain2id, dep_embedding,
                          add_domain_feat=False, reduce_dim_emb=False, add_pos_feat=True,
                          project_after_concat=False, 
                          use_lap_pe=True, lap_k=8, undirected_for_lap=True,
                          use_degree_pe=True, use_rw_pe=True, rw_steps=(1,2,4,8),
                          use_isg_structural_pe=True, use_token_distance_attn=False,
                          use_global_pooling=False, set="train"):
    """
    Original extract_feat function (copy your existing one here)
    This is kept as backup in case you need to disable caching
    """
    # PASTE YOUR ORIGINAL EXTRACT_FEAT FUNCTION BODY HERE
    pass
...
if __name__ == "__main__":
    ...
    if args.config_path:
        ...
        config = {
            ...
            'gnn_type': 'Hybrid-ISG-GTN', # Vanilla-GTN-NoEdge', Vanilla-GTN-EdgeConcat, ISG-VanillaGTN, Hybrid-ISG-GTN, TransformerConv
            'norm_type': 'batchnorm',        # 'batchnorm', 'layernorm', or None
