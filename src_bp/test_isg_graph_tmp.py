...
def log_conf_matrix(y_pred, y_true, normalize=True):
    ...
    cfm_plot = sns.heatmap(df_cfm, annot=True, cmap='Blues', fmt=fmt)
    plt.ylabel('True label')
    plt.xlabel('Predicted label')
    plt.title('Normalized Confusion Matrix' if normalize else 'Confusion Matrix')
    plt.tight_layout()
    save_path = f'{utils.OUTPUT_DIR_PATH}/images/cm.png'
...
class GNN(nn.Module):
    def __init__(
        ...
        self.attn_weights_all = []  # store attention weights per layer
    ...
    def get_graph_embedding(self, x, edge_index, edge_attr=None, batch=None, return_attention=False):
        self.attn_weights_all = []  # reset on each forward

        if isinstance(self.conv1, TransformerConv):
            x, attn = self.conv1(x, edge_index, edge_attr, return_attention_weights=True)
            self.attn_weights_all.append(attn[1])  # edge attention weights
        else:
            x = self.conv1(x, edge_index, edge_attr if self.edge_attr else None)
        ...
        for i in range(self.num_layers):
            ...
            if isinstance(conv, TransformerConv):
                ...
                self.attn_weights_all.append(attn[1])
            else:
                x = conv(x, edge_index, edge_attr if self.edge_attr else None)
            x = F.relu(x)
            x = self.norms[i](x)
            x = F.dropout(x, p=self.dropout, training=self.training)

        x = self.global_pool(x, batch)
        return (x, self.attn_weights_all) if return_attention else x

    def forward(self, x, edge_index, edge_attr=None, batch=None, return_attention=False):
        out = self.get_graph_embedding(x, edge_index, edge_attr, batch, return_attention=return_attention)
        if return_attention:
            x, attn_weights = out
...
def test_gnn(model, device, loader, criterion, extract_attention=False):
    model.eval()
    all_loss = 0.0
    correct = 0
    all_preds = []
    ...
    for data in loader:
        ...
        with torch.no_grad():
            if extract_attention:
                out, attn_weights = model(data.x, data.edge_index, data.edge_attr, data.batch, return_attention=True)
                attention_by_doc.extend([
                    {
                        "edge_index": data.edge_index.cpu().numpy(),
                        "attn_weights": [att.cpu().numpy() for att in attn_weights],
                        "batch": data.batch.cpu().numpy(),
                    }
                ] * data.num_graphs)  # one per graph in batch
            else:
    ...
    if extract_attention:
        return f1_macro, accuracy, all_loss / len(loader), all_preds, all_labels, attention_by_doc
    else:
...
def get_top_k_attended_words(graph, edge_index, attn_weights, k=5):
    edge_attentions = attn_weights[1]  # Use first GTN layer
    node_list = list(graph.nodes())    # Avoid repeated conversion

    # Efficient top-k selection
    top_edges = heapq.nlargest(k, zip(edge_index.T, edge_attentions), key=lambda x: x[1])

    words = []
    for (src, tgt), score in top_edges:
        try:
            src_node = node_list[src]
            tgt_node = node_list[tgt]
            words.append((src_node, tgt_node, score))
        except IndexError:
            continue
    return words


def analyze_attention(topk_insights, stop_pos_tags=None, top_k=20):
    """
    Analyze attention insights by:
    - Filtering out stopword POS categories
    - Averaging attention scores per word
    - Grouping by class label (human=0, machine=1)
    """
    if stop_pos_tags is None:
        stop_pos_tags = {'DET', 'ADP', 'AUX', 'CCONJ', 'PUNCT', 'SCONJ', 'PART'}

    word_attention_by_class = {0: defaultdict(list), 1: defaultdict(list)}

    for doc in topk_insights:
        label = doc["true_label"]
        for src, tgt, score in doc["top_attended_edges"]:
            # Filter out stopword-like POS tags
            if isinstance(score, (list, tuple)):
                score = score[0]
            src_word, src_pos = src.rsplit("_", 1)
            tgt_word, tgt_pos = tgt.rsplit("_", 1)

            if src_pos not in stop_pos_tags:
                word_attention_by_class[label][src].append(score)
            if tgt_pos not in stop_pos_tags:
                word_attention_by_class[label][tgt].append(score)

    # Normalize by average attention per word
    top_words_by_class = {}
    for label, word_dict in word_attention_by_class.items():
        avg_attention = {
            word: sum(scores) / len(scores)
            for word, scores in word_dict.items()
        }
        top_words_by_class[label] = sorted(avg_attention.items(), key=lambda x: x[1], reverse=True)[:top_k]
...
def main(dataset_name, cut_off_dataset, cuda_num=0, 
    ...
    if build_graph: 
        ...
        utils.save_data(graph_info_data, file_name_data, path=f'{output_dir}/', format_file='.pkl', compress=False)

    else:
        graph_info_data = utils.load_data(file_name_data, path=f'{output_dir}/', format_file='.pkl', compress=False)
        data_train_list = graph_info_data["data_train_list"]
        data_val_list = graph_info_data["data_val_list"]
        data_test_list = graph_info_data["data_test_list"]
    ...
    # ********************************************************************************* get_top_k_attended_words
    print("***********************************************************")
    print(graph_info_data.keys())

    test_f1_macro, test_accuracy, test_loss, preds_test, labels_test, attention_by_doc = test_gnn(
        model, device, test_loader, criterion, extract_attention=True)

    print("attention_by_doc: ", len(attention_by_doc))
    topk_insights = []
    for i, attn_doc in enumerate(tqdm(attention_by_doc[:500], desc="get_top_k_attended_words: ")):
        edge_index = attn_doc["edge_index"]
        attn_weights = attn_doc["attn_weights"]
        graph = graph_info_data['graph_test'][i]["graph"]  # your ISG graph for this doc
        top_edges = get_top_k_attended_words(graph, edge_index, attn_weights, k=10)
        
        topk_insights.append({
            "doc_id": i,
            "true_label": labels_test[i],
            "pred_label": preds_test[i],
            "top_attended_edges": top_edges
        })

    print("topk_insights: ", len(topk_insights))

    top_k=50
    top_words_by_class = analyze_attention(topk_insights, stop_pos_tags={}, top_k=top_k)

    # Print results
    print(f"Top-{top_k} Attended Words - Human:")
    for word, count in top_words_by_class.get(0, []):
        print(f"{word}: {count}")

    print("f\nTop-{top_k} Attended Words - Machine:")
    for word, count in top_words_by_class.get(1, []):
        print(f"{word}: {count}")

    summary_path = "top_k_attended_words.txt"
    with open(summary_path, "w", encoding="utf-8") as f:
        f.write(f"\n=== Top-{top_k} Attended Words - Human: ===\n")
        for word, count in top_words_by_class.get(0, []):
            f.write(f"{word} \n")
        f.write(f"\n=== Top-{top_k} Attended Words - Machine: ===\n")
        for word, count in top_words_by_class.get(1, []):
    ...
    # ********************************************************************************* error_analysys
    cut_off_test = int(cut_off_dataset.split('_')[2])
    test_text_set = test_text_set[:int(len(test_text_set) * (cut_off_test / 100))][:]

    print("***********************************************************")
    errors = []
    for i, (pred, true) in enumerate(zip(preds_test, labels_test)):
        if pred != true:
            errors.append({
                "doc_id": i,
                "true_label": true,
                "pred_label": pred,
                "source": test_text_set.iloc[i]["source"],
                "model": test_text_set.iloc[i]["model"],
                "text": test_text_set.iloc[i]["text"],
                "graph": graph_info_data['graph_test'][i]['graph'],
                "top_attended_edges": topk_insights[i]["top_attended_edges"] if i < len(topk_insights) else []
            })


    mistake_types = Counter((e["true_label"], e["pred_label"]) for e in errors)
    print("Mistake Types (true → predicted):", mistake_types)

    print("Errors by Source:")
    error_by_source = Counter([e["source"] for e in errors])
    print(error_by_source)

    print("\nErrors by Model:")
    error_by_model = Counter([e["model"] for e in errors])
    print(error_by_model)

    error_analysis_path = "error_analysis_summary.txt"

    with open(error_analysis_path, "w", encoding="utf-8") as f:
        f.write("=== Mistake Types (true_label → pred_label) ===\n")
        for (true_label, pred_label), count in mistake_types.items():
    ...
    os.remove(error_analysis_path)

    for e in errors[:5]:
        print(f"\nDoc ID: {e['doc_id']}")
        print(f"True: {e['true_label']} | Predicted: {e['pred_label']}")
        print("Text:", e["text"])
        print("source:", e["source"])
        print("model:", e["model"])
        print("Top Attended Edges:", len(["top_attended_edges"]))
        #for src, tgt, score in e["top_attended_edges"]:
        #    print(f"  {src} → {tgt} ({score:.4f})")



    # Assume test_set contains 'labels_test', 'preds_test', 'source', and 'model'
    test_text_set["true_label"] = labels_test
    test_text_set["pred_label"] = preds_test
    test_text_set["correct"] = test_text_set["true_label"] == test_text_set["pred_label"]
    error_by_source = (
        test_text_set.groupby("source")["correct"]
    ...
    error_by_model = (
        test_text_set.groupby("model")["correct"]
        .apply(lambda x: 1 - x.mean())
        .reset_index(name="error_rate")
        .sort_values(by="error_rate", ascending=False)
    )
    ...
    error_by_source_model = (
        test_text_set.groupby(["source", "model"])["correct"]
        .apply(lambda x: 1 - x.mean())
        .reset_index(name="error_rate")
        .sort_values(by="error_rate", ascending=False)
    )
    print(error_by_source_model)
    
    error_by_label_source = (
        test_text_set.groupby(["label", "source"])["correct"]
        .apply(lambda x: 1 - x.mean())
        .reset_index(name="error_rate")
        .sort_values(by="error_rate", ascending=False)
    )
    print(error_by_label_source)

    error_summary_path = "error_rate_summary.txt"

    with open(error_summary_path, "w", encoding="utf-8") as f:
        f.write("=== Error Rate by Model ===\n")
        f.write(error_by_model.to_string(index=False))
        f.write("\n\n=== Error Rate by Source ===\n")
        f.write(error_by_source.to_string(index=False))
        f.write("\n\n=== Error Rate by Source × Model ===\n")
        f.write(error_by_source_model.to_string(index=False))
        f.write("\n\n=== Error Rate by Label × Source ===\n")
        f.write(error_by_label_source.to_string(index=False))

    mlflow.log_artifact(error_summary_path)
    os.remove(error_summary_path)

    # ********************************************************************************* len text/graph analysys 

    analysis_rows = []
    for i, entry in enumerate(graph_info_data['graph_test']):
        text = test_text_set.iloc[i]["text"]
        graph = entry["graph"]
        ...
        analysis_rows.append({
            ...
            "true_label": labels_test[i],
            "pred_label": preds_test[i],
            "correct": labels_test[i] == preds_test[i],
            "text_len": len(text.split()),  # word count
    ...
    summary = df_analysis.groupby("correct")[["text_len", "num_nodes", "num_edges"]].describe()
    print(summary)
    print(df_analysis.corr(numeric_only=True))

    df_analysis.to_csv("graph_textlen_error_analysis.csv", index=False)
    mlflow.log_artifact("graph_textlen_error_analysis.csv")
